...
## Git
`.gitignore` já ignora `target/`, `.idea/`, `*.iml`, `db.properties`, `.env`.

Commits sugeridos (Conventional Commits):
```
chore: inicializa projeto
feat: adiciona modelo do catalogo
feat: implementa conexao com mysql
feat: implementa dao
feat: implementa service
feat: implementa cadastro/listagem/edicao/exclusao
feat: adiciona pesquisa e filtros
test: adiciona testes unitarios
docs: adiciona javadoc e readme
```

## Autor
Projeto acadêmico desenvolvido para estudo de Servlets, JSP, JDBC e MVC.
